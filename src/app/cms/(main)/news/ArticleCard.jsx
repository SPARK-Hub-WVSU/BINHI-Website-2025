  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all duration-200 group">
      ...
      <div className="relative h-48 bg-gray-100">
        ...
        {/* Top Story Badge */}
        {article.isTopStory && (
          <div className="absolute top-3 left-3 bg-primary text-white px-2 py-1 rounded-full text-xs font-semibold flex items-center gap-1">
            <StarIconSolid className="size-3" />
            Top Story
          </div>
        )}
        ...
        <div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          <Link
            ...
            className="bg-white/90 backdrop-blur-sm text-primary px-3 py-1 rounded-full text-sm font-medium hover:bg-white transition-colors">
            Edit
          </Link>
          <div className="bg-white/90 backdrop-blur-sm rounded-full">
            <DeleteButton 
              ...
              onDelete={onDelete}
              className="text-red-600 px-3 py-1 text-sm font-medium hover:bg-red-50 rounded-full transition-colors"
            />
          </div>
        </div>
      </div>
